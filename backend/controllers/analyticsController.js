...
const getEmployerAnalytics = async (req, res) => {
  try {
    ...
    const activeJobLast7Days = await Job.countDocuments({
      company: companyId,
      createdAt: { $gte: last7Days, $lte: now },
    });

    const activeJobPrev7Days = await Job.countDocuments({
      company: companyId,
      createdAt: { $gte: prev7Days, $lte: last7Days },
    });

    const activeJobTrend = getTrend(activeJobLast7Days, activeJobPrev7Days);

    // Application Trend
    const applicationLast7Days = await Application.countDocuments({
      job: { $in: jobIds },
      createdAt: { $gte: last7Days, $lte: now },
    });

    const applicationPrev7Days = await Application.countDocuments({
      job: { $in: jobIds },
      createdAt: { $gte: prev7Days, $lte: last7Days },
    });

    const applicantTrend = getTrend(applicationLast7Days, applicationPrev7Days);

    // Hired Applicants Trend
    const hiredLast7Days = await Application.countDocuments({
      job: { $in: jobIds },
      status: "Accepted",
      createdAt: { $gte: last7Days, $lte: now },
    });

    const hiredPrev7Days = await Application.countDocuments({
      job: { $in: jobIds },
      status: "Accepted",
      createdAt: { $gte: prev7Days, $lte: last7Days },
    });
